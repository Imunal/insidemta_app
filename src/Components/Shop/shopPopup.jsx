...
class shopPopup extends React.Component{
    ...
    stripePromise = loadStripe('pk_live_51HrJxODB7vD9Db3SmMk5R5crpT53hAeo8NzouOE9p2PGJnfAkbt7Bk315LitFiay9hnfchy15fFVbSLgtCe0oWqG00PRkVHgZn');
    ...
    componentDidUpdate(props) {
      ...
      if(this.props.shopSelected !== props.shopSelected) {
        if (this.props.shopSelected > 2)
          this.setState({isPaymentOn: false, selectedPremiumDays: this.PremiumInfo[this.props.shopSelected - 1][0][0]})
        else
          this.setState({isPaymentOn: false, selectedPremiumDays: null})
      }
    }

    popupClose = () => {
      if (this.state.requestOpen && !this.state.requestResult) return;
      this.setState({ isSMSPayment: null, requestOpen: null, requestResult: null, haveError: null, isPaymentOn: false});
            ...
            <div className="form-group">
              <label htmlFor="selectedPremiumDays">Wybierz liczbę dni:</label>
                <div className="dropdown">
                  <select value={this.state.selectedPremiumDays} id="selectedPremiumDays" className="dropdown-select" onChange={this.handelSelectFormChange} disabled={this.state.isPaymentOn || this.state.isSMSPayment ? true : false} required>
                    <option value="">Wybierz...</option>
                    {this.PremiumInfo[this.props.shopSelected - 1].map((data) => (
                    <option value={data[1]} key={data[1]}>{data[1]}dni za {data[0]}zł</option>
                  ))}
                  </select>
                </div>
            </div>
    ...
    getOptionPrice = () => {
        ...
        if(this.props.shopSelected <= 2)
          for (let index = 0; index < this.PremiumInfo[this.props.shopSelected - 1].length; index++) {
          const data = this.PremiumInfo[this.props.shopSelected - 1][index];
            if (data[1] === parseInt(this.state.selectedPremiumDays))
              return data[0]
          }
        else
          return this.state.selectedPremiumDays
    }
    ...
    submitForm = (event) => {
      ...
      axios.post(url, {
        playerName: this.state.playerName
          ...
          if (response.data) {
              this.setState({ isPaymentOn: true, haveError: '', loaderOpen: false });

          }else{
              this.setState({ haveError: "Podany użytkownik nie został odnaleziony.", loaderOpen: false });
          }
      }).catch((error) => {
          this.setState({ haveError: "Podany użytkownik nie został odnaleziony.", loaderOpen: false });
      });
    ...
    getSMSButton = () => {
      ...
      if (this.PremiumInfo[this.props.shopSelected - 1][index][3] === "block") return
    ...
    getSMSPayment = () => {
      ...
      return (
        <>
          ...
          <div className="form-group mb-0 mt-2" style={{ display: this.state.isSMSPayment && !this.state.loaderOpen ? "block" : "none" }}>
      ...
      axios.post(url, {
        ...
        number: smsData[0]
          ...
          if (response.data.status === 1) {
            this.setState({ requestResult: 1, haveError: '', isSMSPayment: false });

          }else{
            this.setState({ haveError: "Podany kod SMS jest niepoprawny.", requestOpen: false });
          }
      }).catch((error) => {
          this.setState({ haveError: "Podany kod SMS jest niepoprawny.", requestOpen: false });
      });
         ...
         <div className={this.props.shopSelected ? "popup showed" : "popup"} >
            <div className="popup_anim">
              <div className="popup_box">
                ...
                <div className="block__center w-100 h-100 mt-5 mb-0" style={{ display: this.state.requestOpen ? this.state.requestResult ? "none" : "block" : "none" }}>
                ...
                <form onSubmit={this.submitForm} style={{display: this.state.requestOpen ? "none" : "block"}}>
                  <div className="form-group">
                    ...
                    <input
                      ...
                      required
                      autoComplete = "off"
                      disabled={this.state.isPaymentOn || this.state.isSMSPayment ? true : false}
                    />
                  </div>

                  {this.getSelectData()}

                  <p style={{ display: this.state.haveError ? "block" : "none" }} className="text-danger text-center m-3">
                    {this.state.haveError}
                  </p>

                  <div className="form-group mb-0 mt-3" style={{ display: this.state.isPaymentOn || this.state.loaderOpen || this.state.isSMSPayment ? "none" : "block" }}>
                    <button type="submit" className="btn btn__dark btn-block">
                ...
                <div className="block__center w-100 h-100 mt-5 mb-0" style={{ display: this.state.loaderOpen ? "block" : "none" }}>
                  <Loader type="Bars" color="#ccc" height={50} width={50} />
                  <p className="text-small text-center text-muted">
                ...
                {this.state.isPaymentOn &&
                  <div className="popup_scroll mt-4" style={{ display: this.state.isPaymentOn ? this.state.requestOpen ? "none" : "block" : "none" }}>
