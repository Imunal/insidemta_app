          ...
          <div className="col-md-4" key={index}>
            <div className="panel__body__element text-center">
              <img
                ...
                loading="lazy"
              />
              <h6 className="mt-3 text-muted text-break">{getVehicleName(vehicle.model)} ({vehicle.ID})</h6>
              <span className="text-muted text-break">Poj. silnika: {vehicle.engineCapacity}</span><br/>
              <span className="text-muted text-break">Przebieg: {Math.floor(vehicle.mileage * 100) / 100}km</span><br/>
              <span className="text-muted text-break">Ulepszenia: {getVehicleUpgrades(vehicle.tuning)}</span>
            </div>
          </div>
        ))}
      </div>
